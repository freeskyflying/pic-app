...
void JT808Activesafety_UploadFile(JT808ActivesafetyUploadHandle_t* handle)
{
	...
	if (pfile != 0)
	{
		...
		while (handle->mState != upload_state_exit)
		{
			...
			int ret = write(handle->socketFd, buffer, (dataLen+62));
			if (ret < 0)
			{
		...
		case 0x9212:
			{
				...
				if (handle->currenFileIdx < handle->currentFileCount)
				{
					usleep(100*1000);
					JT808ActiveSafety_Send1211(handle);
				}
...
void* activesafety_upload_task(void* pArg)
{
	...
	while (session->mRunningFlag)
	{
		...
		if (currentTimeMillis()/1000 < 10 + timeAlert) // 报警附件30s后上传
		{
		...
		PNT_LOGE("alertTime: %lld current: %lld", timeAlert, currentTimeMillis()/1000);
...
reconnect:
		...
		while (handle->mState != upload_state_exit)
		{
			...
			int readLen = recv(handle->socketFd, recvBuffer+dataLen, 512, 0);
...
void JT808ActiveSafety_UploadInit(JT808ActivesafetyUploadHandle_t * handle, void* owner)
{
	...
	queue_list_create(&handle->reqList, 100);
