...
void* jt1078RtspSessionProcessor(void* pArg)
{
	...
	queue_list_create(&rtsp->mFrameListVideo, 4);
	queue_list_create(&rtsp->mFrameListAudio, 4);
	queue_list_set_free(&rtsp->mFrameListVideo, FrameListContentFree);
	queue_list_set_free(&rtsp->mFrameListAudio, FrameListContentFree);
...
connect:
	...
	while (TRUE)
	{
		...
		FrameInfo_t* frame = queue_list_popup(&rtsp->mFrameListVideo);
		if (NULL != frame)
		{
			if (1 == start || 0x67 == frame->frameBuffer[4])
			{
		...
		FrameInfo_t* audioFrame = queue_list_popup(&rtsp->mFrameListAudio);
		if (NULL != audioFrame)
		{
			jt1078RtspPackAndSendAudioRtp(rtsp, (unsigned char*)audioFrame->frameBuffer, audioFrame->frameSize);
		
			FrameListContentFree(audioFrame);
		}
		
		controller->mTimestamp = currentTimeMillis();

		if (NULL == frame && NULL == audioFrame)
		{
			SleepMs(40);
		}
	}
	...
	queue_list_clear(&rtsp->mFrameListVideo);
	queue_list_clear(&rtsp->mFrameListAudio);
	queue_list_destroy(&rtsp->mFrameListVideo);
	queue_list_destroy(&rtsp->mFrameListAudio);
...
int JT1078Rtsp_ChannelCreate(JT1078RtspControl_t* controller, JT808MsgBody_9101_t* param)
{
	...
	if (0 == param->mDataType || 2 == param->mDataType || 3 == param->mDataType)
	{
		streamState |= 0x02;
	}
	if (3 < param->mDataType)
	{
		return RET_FAILED;
	}
...
{	
	...
	while (head)
	{
		...
		if (chnNum == rtsp->mLogicChannel && JT1078RtspStatePlaying == rtsp->mState && rtsp->mStreamType == sub && (rtsp->mStreamState&0x01))
		{
			if (rtsp->mFrameListVideo.m_init)
			{
				FrameInfo_t* frame = (FrameInfo_t*)malloc(sizeof(FrameInfo_t));
				frame->frameSize = len1 + len2;
				frame->timeStamp = currentTimeMillis();
				frame->frameBuffer = (char*)malloc(frame->frameSize);
				if (NULL != frame->frameBuffer)
				{
					memcpy(frame->frameBuffer, buff1, len1);
				    if (buff2 && len2 > 0)
					    memcpy(frame->frameBuffer+len1, buff2, len2);

				    if (queue_list_push(&rtsp->mFrameListVideo, frame))
				    {
				    	FrameListContentFree(frame);
				    }
				}
...
{	
	...
	while (head)
	{
		...
		if (/*chnNum == rtsp->mLogicChannel &&*/ JT1078RtspStatePlaying == rtsp->mState && (rtsp->mStreamState&0x02))
		{
			if (rtsp->mFrameListAudio.m_init)
			{
				FrameInfo_t* frame = (FrameInfo_t*)malloc(sizeof(FrameInfo_t));
				frame->frameSize = len;
				frame->timeStamp = currentTimeMillis();
				...
				if (NULL != frame->frameBuffer)
				{
					memcpy(frame->frameBuffer, buff, len);

				    if (queue_list_push(&rtsp->mFrameListAudio, frame))
				    {
				    	FrameListContentFree(frame);
				    }
				}
