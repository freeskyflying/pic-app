...
speech_get_token(const char *api_key, const char *secret_key, const char *scope, char *token)
{
    ...
    PNT_LOGI("the url to get the token are :%s", url);
    ...
    {
        PNT_LOGE("fatal exception happened!!! fail to get the token from:%s with result:%d",
                url, res_curl);
    ...
    {
        PNT_LOGE("success to get the token!!!");
        res = parse_token(response, scope, token); // 解析token，结果保存在token里
        if (res == RETURN_OK)
        {
            PNT_LOGE("the token we get are:%s with response:%s", token, response);
        }
    }
    free(response);
