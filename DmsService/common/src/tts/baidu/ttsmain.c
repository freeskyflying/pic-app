...
RETURN_CODE fill_config(char* argv,struct tts_config *config)
{
    ...
    char api_key[] = "uZe8aplpqqhTEvNzGHCNHV2m";
    // 填写网页上申请的APP SECRET 如 $secretKey="94dc99566550d87f8fa8ece112xxxxx"
    char secret_key[] = "fbPhfQEwGaj9GcCN5hruwDrv2YjwhVgd";
...
//，SD2故障，硬盘故障，U盘故障
    ...
    config->vol = vol;
	config->aue = aue;
	sprintf(config->downloadPath, "%s", "/cache/result.mp3");
	// aue对应的格式，format
...
RETURN_CODE run_tts(TTSMain_t* tts, struct tts_config *config, const char *token)
{
    ...
    PNT_LOGE("download the recognized pcm file from --> :%s", url);
    curl_free(cuid);
  	curl_free(tex);
    ...
    {
        PNT_LOGE("unexpected exception happened!!! fail to download %s from %s, with ret:%d", config->downloadPath, url,
                res);
        // curl 失败
    ...
    {
        PNT_LOGE(" --> download %s from %s success!!!", config->downloadPath, url);
    }
	if (result.fp != NULL)
    {
		fclose(result.fp);
	...
	if(tts->mCb != NULL)
    {
        PNT_LOGE("notity download:%s success", config->downloadPath);
...
void* ttsThreadLoop(void* arg)
{
    ...
    PNT_LOGE("--> the tts content are --> %s", tts->mText);
    ...
    PNT_LOGE("the tts remote server config result are --> %d", res);
    if (res == RETURN_OK) {
        // 获取token
        PNT_LOGE("start to request the token --> ");
        res = speech_get_token(config.api_key, config.secret_key, TTS_SCOPE, token);
        PNT_LOGE("the result of get speech token are --> %d", res);
        if (res == RETURN_OK) {
            // 调用识别接口
            PNT_LOGE("start to perform recognize with token --> %s", token);
            run_tts(tts, &config, token);
        } else {
            PNT_LOGE("fatal exception!!! fail to get the speech recognize token with ret:%d!!!", res);
        }
    } else {
        PNT_LOGE("fatal state exception!!! fail to perform the config with ret:%d", res);
    }
    curl_global_cleanup();
    free(tts->mText);
    tts->mText = NULL;
...
int TTSMain_Start(TTSMain_t* tts, char* text, ttsCompleteCallback cb)
{
    ...
    tts->mCb = cb;
    tts->mText = (char*)malloc(strlen(text) + 1);
    if(tts->mText != NULL){
        memset(tts->mText, 0, strlen(text + 1));
