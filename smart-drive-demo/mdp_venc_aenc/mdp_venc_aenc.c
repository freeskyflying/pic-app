...
//#define SENSOR_TYPE IMX327_MIPI_1920_1080_30FPS_RAW12
#define SENSOR_TYPE IMX307_MIPI_1920_1080_30FPS_RAW12

#if 1
#define SDR_VISS_SNS_DVP TP9930_DVP_1920_1080_30FPS_4CH_YUV
#define SDR_VISS_SNS_MIPI CLB_1280_720_90FPS_YUV
...
#define SDR_VISS_FRAMERATE 25
#endif
#define MAIN_STREAM_NUMBER 1
#define SUB_STREAM_NUMBER 1
...
void _fallocate_stream_file(sdr_sample_venc_para_t *sdr_venc_param, int number_stream, int venc_time)
{
    ...
    for (i = 0; i < number_stream; i++) {
        ...
        size = venc_para->venc_info.bitrate * venc_time / 8 + 16 * 1024 *1024;
...
{
    sdr_sample_venc_para_t *sdr_venc_para = hdl;
    
    sprintf(filename, "/data/rec_%05d.mp4", sdr_venc_para->file_cnt);
    sdr_venc_para->file_cnt++;
    return 0;
}
...
static int _file_closed(void *hdl, char *filename)
{
    ...
    if (sdr_venc_para->file_cnt > 240) {
        char name[128];
        sprintf(name, "/data/rec_%05d.mp4", sdr_venc_para->rm_file_cnt);
        unlink(name);
        sdr_venc_para->rm_file_cnt++;
    }
    return 0;
}

sample_muxer_hdl_t *g_muxer_hdl[3];

void *sample_muxer_init(sdr_sample_venc_para_t *sdr_venc_para,int mode, char *filename)
{
    ...
    if (sdr_venc_para->venc_info.enType == PT_H265)
        muxer_hdl->muxer_parm.stream_parm[VIDEO_INDEX].codec_id = AV_CODEC_TYPE_HEVC;
    else if (sdr_venc_para->venc_info.enType == PT_H264)
    ...
    muxer_hdl->muxer_parm.stream_parm[VIDEO_INDEX].width = sdr_venc_para->venc_info.width;
    muxer_hdl->muxer_parm.stream_parm[VIDEO_INDEX].height = sdr_venc_para->venc_info.hight;

    muxer_hdl->muxer_parm.stream_parm[AUDIO_INDEX].codec_type = AV_MEDIA_TYPE_AUDIO;
    if (sdr_venc_para->aenc_codec == PT_AAC)
        muxer_hdl->muxer_parm.stream_parm[AUDIO_INDEX].codec_id = AV_CODEC_TYPE_AAC;
    else if (sdr_venc_para->aenc_codec == PT_G729)
        muxer_hdl->muxer_parm.stream_parm[AUDIO_INDEX].codec_id = AV_CODEC_TYPE_G729;
    else if (sdr_venc_para->aenc_codec == PT_LPCM)
        muxer_hdl->muxer_parm.stream_parm[AUDIO_INDEX].codec_id = AV_CODEC_TYPE_PCM_S16LE;
    else if (sdr_venc_para->aenc_codec == PT_ADPCMA)
        muxer_hdl->muxer_parm.stream_parm[AUDIO_INDEX].codec_id = AV_CODEC_TYPE_PCM_ALAW;
    muxer_hdl->muxer_parm.stream_parm[AUDIO_INDEX].sample_rate = sdr_venc_para->aenc_rate;
    muxer_hdl->acodec_id = muxer_hdl->muxer_parm.stream_parm[AUDIO_INDEX].codec_id;

    muxer_hdl->muxer_parm.fix_duration_param.app_data = sdr_venc_para;
    muxer_hdl->muxer_parm.fix_duration_param.cb_get_next_file = _get_filename;
...
static inline int save_stream_init(sdr_sample_venc_para_t *sdr_venc_para)
{
    ...
    if (type == AV_MUXER_TYPE_TS || type == AV_MUXER_TYPE_MP4) {
        printf("type == AV_MUXER_TYPE_TS || type == AV_MUXER_TYPE_MP4\n");
        sdr_venc_para->muxer_hdl = sample_muxer_init(sdr_venc_para,
            type, aszFileName);
        if (sdr_venc_para->muxer_hdl)
            ret = 0;
    }

    snprintf(aszFileName, 128, PATH_ROOT"/test1_chn%02d-%03d%s",
        sdr_venc_para->VeChn,
        sdr_venc_para->file_idx, szFilePostfix);

    if (type == AV_MUXER_TYPE_TS || type == AV_MUXER_TYPE_MP4) {
        printf("type == AV_MUXER_TYPE_TS || type == AV_MUXER_TYPE_MP4\n");
        g_muxer_hdl[0] = sample_muxer_init(sdr_venc_para,
            type, aszFileName);
        if (sdr_venc_para->muxer_hdl)
            ret = 0;
    }

    snprintf(aszFileName, 128, PATH_ROOT"/test2_chn%02d-%03d%s",
        sdr_venc_para->VeChn,
        sdr_venc_para->file_idx, szFilePostfix);

    if (type == AV_MUXER_TYPE_TS || type == AV_MUXER_TYPE_MP4) {
        printf("type == AV_MUXER_TYPE_TS || type == AV_MUXER_TYPE_MP4\n");
        g_muxer_hdl[1] = sample_muxer_init(sdr_venc_para,
            type, aszFileName);
        if (sdr_venc_para->muxer_hdl)
            ret = 0;
    }

    snprintf(aszFileName, 128, PATH_ROOT"/test3_chn%02d-%03d%s",
        sdr_venc_para->VeChn,
        sdr_venc_para->file_idx, szFilePostfix);

    if (type == AV_MUXER_TYPE_TS || type == AV_MUXER_TYPE_MP4) {
        printf("type == AV_MUXER_TYPE_TS || type == AV_MUXER_TYPE_MP4\n");
        g_muxer_hdl[2] = sample_muxer_init(sdr_venc_para,
            type, aszFileName);
        if (sdr_venc_para->muxer_hdl)
            ret = 0;
    }
...
static int save_vstream(sdr_sample_venc_para_t *sdr_venc_para, VENC_STREAM_S *pstStream)
{
    int ret = -1;
    ...
    if (sdr_venc_para->muxer_type == AV_MUXER_TYPE_TS ||
        ...
        ret = sample_muxer_write_packet(g_muxer_hdl[0], pstStream);
        ret = sample_muxer_write_packet(g_muxer_hdl[1], pstStream);
        ret = sample_muxer_write_packet(g_muxer_hdl[2], pstStream);
        }
#endif
    return ret;
}
static int save_astream(sdr_sample_venc_para_t *sdr_venc_para, AUDIO_STREAM_S *pstStream)
{
    int ret = 0;
    if (pstStream == NULL || sdr_venc_para == NULL)
        return EI_FAILURE;
    ...
    if (sdr_venc_para->muxer_type == AV_MUXER_TYPE_TS ||
        sdr_venc_para->muxer_type == AV_MUXER_TYPE_MP4)
        ret = sample_muxer_write_aenc_packet(sdr_venc_para->muxer_hdl, pstStream);
...
#if 0
static int sdrive_message_init(_queue_t *queue)
{
    int ret;
    EI_PRINT("%s,%d\n", __func__, __LINE__);
    ...
    queue->head = (_message_t *)malloc(sizeof(_message_t));
    if (!(queue->head))
        return -2;

    memset(queue->head, 0, sizeof(_message_t));
    queue->tail = queue->head;
    EI_PRINT("%s,%d, queue:%p,queue->tail:%p\n", __func__, __LINE__, queue, queue->tail);

    queue->count = 0;
    return 0;
}

void sdrive_message_deinit(_queue_t *queue)
{
    _message_t *tmp;
...
static int sdrive_message_push(_queue_t *queue, _message_t *msg_in)
{
    _message_t *new_msg;
    int ret = 0;
    ...
    new_msg = malloc(sizeof(_message_t));
    if (new_msg == NULL) {
        ret = -3;
        goto EXIT;
    }
    memcpy(new_msg, msg_in, sizeof(_message_t));
...
static int sdrive_message_pop(_queue_t *queue, _message_t *msg_out)
{
    _message_t *tmp;
    int ret = 0;
    ...
    tmp = queue->head;
    memcpy(msg_out, tmp->next, sizeof(_message_t));
    queue->head = tmp->next;
    free(tmp);
    queue->count--;
    EXIT:
...
#endif
#if 0
EI_VOID *_get_aenc_stream_proc(EI_VOID *p)
{
    sdr_sample_venc_para_t *sdr_venc_para;
    VC_CHN AencChn;
    EI_S32 AencFd;
    _message_t message = {0};
    int ret;
    char thread_name[16];
    EI_S32 s32Ret;
    AUDIO_STREAM_S stStream;
    AUDIO_STREAM_S *pstStream = NULL;
    fd_set read_fds;
    ...
    AencChn = sdr_venc_para->aenc_chn;

    thread_name[15] = 0;
    snprintf(thread_name, 16, "astreamproc%d", AencChn);
    ...
    while (EI_TRUE == sdr_venc_para->bThreadStart) {
        if (sdr_venc_para->aenc_queue.count > 0) {
            usleep(10 * 1000);
            continue;
        }
        TimeoutVal.tv_sec = 1;
        ...
        if (FD_ISSET(AencFd, &read_fds)) {
            ...
            s32Ret = EI_MI_AENC_GetStream(AencChn, &stStream, -1);
            if (EI_SUCCESS != s32Ret) {
                EI_PRINT("%s: EI_MI_AENC_GetStream(%d), failed with %#x!\n", \
                    __FUNCTION__, AencChn, s32Ret);
            }
            EI_PRINT("get:%llu,addr:%p\n", stStream.u64TimeStamp, stStream.pStream);
            //push buffer;
            if (sdr_venc_para->init_flag) {
                pstStream = malloc(sizeof(AUDIO_STREAM_S));
                if (pstStream) {
                    memcpy(pstStream, &stStream, sizeof(AUDIO_STREAM_S));
                    message.data = pstStream;
                    ret = sdrive_message_push(&sdr_venc_para->aenc_queue, &message);
                    
                    if (ret) {
                        EI_PRINT("push abuffer err(%d)!", ret);
                    }
                }
            } //else
...
EI_VOID *_get_aenc_stream_proc(EI_VOID *p)
{
    ...
    AeChn = sdr_venc_para->aenc_chn;

    thread_name[15] = 0;
    snprintf(thread_name, 16, "streammux%d", VencChn);
    prctl(PR_SET_NAME, thread_name);
    FD_ZERO(&read_fds);
    AencFd = EI_MI_AENC_GetFd(AencChn);
    FD_SET(AencFd, &read_fds);
    ...
    while (EI_TRUE == sdr_venc_para->bThreadStart) {
        ...
        if (FD_ISSET(AencFd, &read_fds)) {
            ...
            if (!sdr_venc_para->init_flag) {
                EI_MI_AENC_ReleaseStream(AencChn, &stStream);
                continue;
            }
        }
        ...
        if (stStream.pStream) {
            ...
            ret = save_astream(sdr_venc_para, &stStream);
            pthread_mutex_unlock(&sdr_venc_para->mutex);
            if (ret)
                EI_PRINT("%s,%d save astream err!\n", __func__, __LINE__);

            ret = EI_MI_AENC_ReleaseStream(AeChn, &stStream);
            if (ret != EI_SUCCESS) {
...
EI_VOID *_get_venc_stream_proc(EI_VOID *p)
{
    ...
    thread_name[15] = 0;
    snprintf(thread_name, 16, "vstreamproc%d", VencChn);
    prctl(PR_SET_NAME, thread_name);
    while (EI_TRUE == sdr_venc_para->bThreadStart) {
        memset(&stStream, 0, sizeof(VENC_STREAM_S));
        ret = EI_MI_VENC_GetStream(VencChn, &stStream, 100);
        if (ret == EI_ERR_VENC_NOBUF) {
            EI_PRINT("EI_ERR_VENC_NOBUF\n");
            usleep(5 * 1000);
            continue;
        } else if (ret != EI_SUCCESS) {
            EI_PRINT("get chn-%d error : %d\n", VencChn, ret);
            break;
        }
        //EI_PRINT("vstream:%llu,%llu\n",stStream.pstPack.u64PTS, stStream.pstPack.u64PhyAddr[0]);
        ...
        if (sdr_venc_para->init_flag == 0) {
            printf("sdr_venc_para->mux:%d\n",sdr_venc_para->muxer_type);
            save_stream_init(sdr_venc_para);
        }
        if (sdr_venc_para->init_flag) {
            pthread_mutex_lock(&sdr_venc_para->mutex);
            ret = save_vstream(sdr_venc_para, &stStream);
            pthread_mutex_unlock(&sdr_venc_para->mutex);
            if (ret)
                EI_PRINT("%s,%d save vstream err!\n", __func__, __LINE__);
        }
        ret = EI_MI_VENC_ReleaseStream(VencChn, &stStream);
            if (ret != EI_SUCCESS) {
                EI_PRINT("release stream chn-%d error : %d\n",VencChn, ret);
                break;
            }
        encoded_packet++;
    }
...
int _start_get_stream(sdr_sample_venc_para_t *venc_para)
{
    ...
    ret = pthread_create(&venc_para->gs_AencPid, 0, _get_aenc_stream_proc,
            (EI_VOID *)venc_para);
    if (ret)
        EI_PRINT("errno=%d, reason(%s)\n", errno, strerror(errno));
...
int _stop_get_stream(sdr_sample_venc_para_t *venc_para)
{
    ...
    if (venc_para->gs_AencPid) {
        pthread_join(venc_para->gs_AencPid, 0);
        venc_para->gs_AencPid = 0;
    }
    return EI_SUCCESS;
}
...
static int _bufpool_init_1ch(VBUF_CONFIG_S *vbuf_config, EI_U32 width, EI_U32 hight)
{
    memset(vbuf_config, 0, sizeof(VBUF_CONFIG_S));
    ...
    vbuf_config->astFrameInfo[0].stCommFrameInfo.enCompressMode = COMPRESS_MODE_NONE;
    if (width == 1920 && hight == 1080) {
        vbuf_config->astCommPool[0].u32BufCnt = 10;
        vbuf_config->u32PoolCnt = 1;
    } else {
        vbuf_config->astCommPool[0].u32BufCnt = 10;
        vbuf_config->u32PoolCnt = 2;
        ...
        vbuf_config->astFrameInfo[1].stCommFrameInfo.u32Height = hight;
        vbuf_config->astFrameInfo[1].stCommFrameInfo.enPixelFormat = PIX_FMT_YUV_SEMIPLANAR_420;
        vbuf_config->astCommPool[1].u32BufCnt = 10;
        vbuf_config->astCommPool[1].enRemapMode = VBUF_REMAP_MODE_NOCACHE;
    }
...
static int _start_viss_1ch(SAMPLE_VISS_CONFIG_S *viss_config)
{
    SAMPLE_VISS_INFO_S *viss_info;
...
//    viss_info->stDevInfo.aBindPhyChn[0] = 0;
    ...
    viss_info->stChnInfo.stChnAttr.u32Align = 32;
//    viss_info->stChnInfo.stChnAttr.u32Depth = 2;
    viss_info->stSnsInfo.enSnsType = SENSOR_TYPE;

    //SAMPLE_COMM_VISS_GetPicTypeBySensor(SDR_VISS_SNS_MIPI, &stPicType);
    //SAMPLE_COMM_VISS_GetChnAttrBySns(SDR_VISS_SNS_MIPI, &stVissChnAttr);
    ...
    COMMON_FRAME_INFO_S *out_frame_info)
{
    ...
    ippu_chn_attr[0].u32Width	= 1920;
    ippu_chn_attr[0].u32Height	= 1080;
    ippu_chn_attr[0].u32Align	= out_frame_info->u32Align;
    ippu_chn_attr[0].enPixelFormat = PIX_FMT_YUV_SEMIPLANAR_420;
    ippu_chn_attr[0].stFrameRate.s32SrcFrameRate = viss_config->astVissInfo[0].stIspInfo.stFrc.s32DstFrameRate;
    ippu_chn_attr[0].stFrameRate.s32DstFrameRate = viss_config->astVissInfo[0].stIspInfo.stFrc.s32DstFrameRate;
    ippu_chn_attr[0].u32Depth	= 0;
...
static int _start_ippu_1ch(EI_BOOL *chn_enabled,
	int num_stream,
	SAMPLE_VISS_CONFIG_S *viss_config,
	COMMON_FRAME_INFO_S *in_frame_info,
	COMMON_FRAME_INFO_S *out_frame_info)
    ...
    SAMPLE_VENC_CONFIG_S *venc_config)
{
    ...
    for (i = 0; i < num_stream; i++) {
        ...
        if (i > 3) {
			...
			printf("=====viss config here viss_dev:%d,viss_chn:%d\n", viss_dev, viss_chn);
        }
...
#if 0
        stream_para[i].aenc_rate = 16000;
        stream_para[i].ai_chn = 0;
        stream_para[i].aenc_chn = 0;
        stream_para[i].aenc_codec = PT_AAC;
        Sample_AI_AENC_AEC(NULL, stream_para[i].aenc_rate, 0, stream_para[i].aenc_codec,
            stream_para[i].ai_chn, stream_para[i].aenc_chn);
#endif
    ...
    SAMPLE_VENC_CONFIG_S *venc_config)
{
    ...
    for (i = 0; i < num_stream; i++) {
        ...
        ret = SAMPLE_COMM_IPPU_Link_VPU(i, 0, stream_para[i].VeChn);
        if (EI_SUCCESS != ret) {
            EI_PRINT("viss link vpu failed %d-%d-%d \n",
    ...
    int num_stream)
{
    ...
    for (i = 0; i < num_stream; i++) {
        stream_para[i].bThreadStart = EI_FALSE;
        _stop_get_stream(&stream_para[i]);
        SAMPLE_COMM_IPPU_UnLink_VPU(i, 0, stream_para[i].VeChn);
        SAMPLE_COMM_VENC_Stop(stream_para[i].VeChn);
        Stop_AI_AENC_AEC(NULL, stream_para[i].aenc_rate, 0, stream_para[i].aenc_codec,
            stream_para[i].ai_chn, stream_para[i].aenc_chn);
    }
    for (i = 0; i < num_stream; i++) {
        save_stream_deinit(&stream_para[i]);
...
int mdp_venc_8ch(video_encode_info_t *vencInfo)
{
	...
	printf("vencInfo->width:%d, vencInfo->hight:%d\n",vencInfo->width, vencInfo->hight);
    ...
    for (i = 0; i < 8; i++) {
		if (isUseVissDataEnc) {
            ...
            main_stream_para[i].muxer_type = vencInfo->MuxerType;
			memcpy(&main_stream_para[i].venc_info, vencInfo, sizeof(video_encode_info_t));
        } else {
            sub_stream_para[i].VeChn = i;
            sub_stream_para[i].file_idx = 0;
            sub_stream_para[i].muxer_type = vencInfo->MuxerType;
			memcpy(&sub_stream_para[i].venc_info, vencInfo, sizeof(video_encode_info_t));
        }
    }
...
exit0:
    ...
    EI_PRINT("The generate files is in /data!\n");
    return 0;
}

int mdp_venc_1ch(video_encode_info_t *vencInfo)
{
    int ret = EI_FAILURE;
    int i = 0;
    static int count = 0;
...
printf("vencInfo->width:%d, vencInfo->hight:%d\n",vencInfo->width, vencInfo->hight);
    ret = _bufpool_init_1ch(&vbuf_config, vencInfo->width, vencInfo->hight);
    if (ret) {
        EI_PRINT("%s %d _bufpool_init err! \n", __func__, __LINE__);
        goto exit0;
    }

    ret = _start_viss_1ch(&viss_config);
    if (ret) {
        EI_PRINT("%s %d _start_viss err! \n", __func__, __LINE__);
        goto exit1;
    }
    ...
    if (isUseVissDataEnc != EI_TRUE) {
        viss_config.astVissInfo[0].stIspInfo.enRunningMode = ISP_MODE_RUNNING_ONLINE;
        viss_config.astVissInfo[0].stIspInfo.stFrc.s32SrcFrameRate = SDR_VISS_FRAMERATE; /* only support offline */
        viss_config.astVissInfo[0].stIspInfo.stFrc.s32DstFrameRate = vencInfo->framerate;
        /*viss_config.astVissInfo[1].stIspInfo.enRunningMode = ISP_MODE_RUNNING_ONLINE;
        viss_config.astVissInfo[1].stIspInfo.stFrc.s32SrcFrameRate = SDR_VISS_FRAMERATE; 
        viss_config.astVissInfo[1].stIspInfo.stFrc.s32DstFrameRate = vencInfo->framerate;*/

        ret = _start_ippu_1ch(chn_enabled, MAIN_STREAM_NUMBER, &viss_config,
                &vbuf_config.astFrameInfo[0].stCommFrameInfo,
                &vbuf_config.astFrameInfo[0].stCommFrameInfo);
        if (ret) {
            EI_PRINT("%s %d _start_ippu err!\n", __func__, __LINE__);
    ...
    for (i = 0; i < 1; i++) {
        if (isUseVissDataEnc) {
            main_stream_para[i].VeChn = i;
            main_stream_para[i].venc_info = *vencInfo;
            main_stream_para[i].file_idx = 0;
            main_stream_para[i].muxer_type = vencInfo->MuxerType;
        } else {
            sub_stream_para[i].VeChn = i;
            sub_stream_para[i].venc_info = *vencInfo;
            sub_stream_para[i].file_idx = 0;
            sub_stream_para[i].muxer_type = vencInfo->MuxerType;
        }
    }
...
exit1:
    _bufpool_deinit(&vbuf_config);
exit0:
    EI_PRINT("1ch video enc end!\n");
    EI_PRINT("The generate files is in /data!\n");
    return 0;
}

static EI_VOID venc_aenc_help(EI_VOID)
{
    printf("Version:1.0.0  \n");
    printf("such as: mdp_venc_aenc -w 1920 -h 1080 -r 25 -t 30 -e 0 -m 0\n");
    printf("such as: mdp_venc_aenc -w 1280 -h 720 -r 25 -t 30 -e 0 -b 1000000\n");
    printf("arguments:\n");
    printf("-w    select video width, support:0~1920\n");
    printf("-h    select video hight, support:0~1080\n");
    printf("-b    select video bitrate, default:8Mbit\n");
    printf("-r    select video record frame rate, default:20, support:1~30\n");
    printf("-e    select enc type, default:h265, support:0.h265,1.h264,2.mjpeg\n");
    printf("-t    select video record time, default:20, support:1~\n");
    printf("-m    select muxer, default:mp4, support:0.mp4,1.ts,2.raw\n");
}
...
int main(int argc, char **argv)
{
    ...
    EI_U32 type = 0; /* 0:h265 1:h264 2.mjpeg*/
    EI_U32 muxer = 0; /* 0:mp4 1:ts 2:raw*/
    int c;
    ...
    vencInfo.width = 1920;
    vencInfo.hight = 1080;
    vencInfo.framerate = 25;
    vencInfo.video_duration = 20;
    vencInfo.bitrate = 8000000;

    while ((c = getopt(argc, argv, "w:h:b:r:e:t:m:v")) != -1) {
        switch (c) {
        case 'w':
            vencInfo.width = (unsigned int)strtol(argv[optind - 1], NULL, 10);
            break;
        case 'h':
            vencInfo.hight = (unsigned int)strtol(argv[optind - 1], NULL, 10);
            break;
        case 'b':
            vencInfo.bitrate = (unsigned int)strtol(argv[optind - 1], NULL, 10);
            break;
        case 'r':
            vencInfo.framerate = (unsigned int)strtol(argv[optind - 1], NULL, 10);
            break;
        case 'e':
            type = (unsigned int)strtol(argv[optind - 1], NULL, 10);
            break;
        case 't':
            vencInfo.video_duration = (unsigned int)strtol(argv[optind - 1], NULL, 10);
            break;
        case 'm':
            muxer = (unsigned int)strtol(argv[optind - 1], NULL, 10);
            break;
        default:
            venc_aenc_help();
            goto EXIT;
    ...
    if (type == 0)
        ...
        vencInfo.enType = PT_MJPEG;

    if (muxer == 0)
        vencInfo.MuxerType = AV_MUXER_TYPE_MP4;
    else if (muxer == 1)
        vencInfo.MuxerType = AV_MUXER_TYPE_TS;
    else if (muxer == 2)
        vencInfo.MuxerType = AV_MUXER_TYPE_RAW;

    if (vencInfo.bitrate == 0)
        vencInfo.bitrate = vencInfo.width * vencInfo.hight * vencInfo.framerate / 10;

    s32Ret = mdp_venc_1ch(&vencInfo);
