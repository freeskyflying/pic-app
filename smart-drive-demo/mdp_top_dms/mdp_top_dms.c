...
#define TOP_DMS_PATH	 "/usr/share/ax/topdms"
...
typedef enum _WARNING_MSG_TYPE_ {
	...
	LB_WARNING_TOP_DMS_NO_FACE,
	LB_WARNING_TOP_DMS_BELT,
	LB_WARNING_TOP_DMS_WHEEL,
	LB_WARNING_TOP_DMS_PLAY_PHONE,
	LB_WARNING_USERDEF,
	LB_WARNING_END,
} WARNING_MSG_TYPE;
...
typedef struct _axframe_info_s {
    ...
    sem_t frame_del_sem;
	nn_top_dms_cfg_t m_top_dms_cfg;
	top_dms_out_t *p_det_info;
	EI_S32 brightness;
	FILE *ax_fout;
} axframe_info_s;
...
{
    printf("usage: mdp_top_dms \n");
    printf("such as for dms recognition: mdp_top_dms\n");
    printf("such as for dms recognition: mdp_top_dms -d 0 -c 0 -r 90\n");
	...
	printf("-b    brightness -64-63, default: 0\n");
    printf("-h    help\n");
}
...
static void top_dms_draw_location_rgn(VIDEO_FRAME_INFO_S *drawFrame, top_dms_out_t *p_det_info)
{
	int draw_num;
	int index = 0;
	...
	draw_num = p_det_info->person.exist + p_det_info->belt.exist +
		p_det_info->wheel.exist + p_det_info->phone.exist;
	...
	if (p_det_info->person.exist) {
		EI_PRINT("person[%d %d %d %d]\n",
			p_det_info->person.box[0], p_det_info->person.box[1],
			p_det_info->person.box[2], p_det_info->person.box[3]);
		loc_x0 = p_det_info->person.box[0];
        loc_y0 = p_det_info->person.box[1];
        loc_x1 = p_det_info->person.box[2];
        loc_y1 = p_det_info->person.box[3];
		drawframe_info.stRectArray[index].stRectAttr.stRect.s32X = loc_x0;
		drawframe_info.stRectArray[index].stRectAttr.stRect.s32Y = loc_y0;
		drawframe_info.stRectArray[index].stRectAttr.stRect.u32Height = loc_y1-loc_y0;
		drawframe_info.stRectArray[index].stRectAttr.stRect.u32Width = loc_x1-loc_x0;
		drawframe_info.stRectArray[index].stRectAttr.u32BorderSize = 4;
		drawframe_info.stRectArray[index].stRectAttr.u32Color = 0xff8000ff;
		drawframe_info.stRectArray[index].isShow = EI_TRUE;
		drawframe_info.stRgnChnAttr[VENC_RGN_WM_NUM].bShow = EI_TRUE;
	...
	index = index + p_det_info->person.exist;
	if (p_det_info->belt.exist) {
		EI_PRINT("belt[%d %d %d %d]\n",
			p_det_info->belt.box[0], p_det_info->belt.box[1],
			p_det_info->belt.box[2], p_det_info->belt.box[3]);
		loc_x0 = p_det_info->belt.box[0];
        loc_y0 = p_det_info->belt.box[1];
        loc_x1 = p_det_info->belt.box[2];
        loc_y1 = p_det_info->belt.box[3];
		drawframe_info.stRectArray[index].stRectAttr.stRect.s32X = loc_x0;
		drawframe_info.stRectArray[index].stRectAttr.stRect.s32Y = loc_y0;
		drawframe_info.stRectArray[index].stRectAttr.stRect.u32Height = loc_y1-loc_y0;
		drawframe_info.stRectArray[index].stRectAttr.stRect.u32Width = loc_x1-loc_x0;
		drawframe_info.stRectArray[index].stRectAttr.u32BorderSize = 4;
		drawframe_info.stRectArray[index].stRectAttr.u32Color = 0xffff0000;
		drawframe_info.stRectArray[index].isShow = EI_TRUE;
		drawframe_info.stRgnChnAttr[VENC_RGN_WM_NUM].bShow = EI_TRUE;
		s32Ret = EI_MI_RGN_SetChnAttr(drawframe_info.Handle[VENC_RGN_WM_NUM], &drawframe_info.stRgnChn[1], &drawframe_info.stRgnChnAttr[VENC_RGN_WM_NUM]);
		if(s32Ret) {
	...
	index = index + p_det_info->belt.exist;
	if (p_det_info->wheel.exist) {
		EI_PRINT("wheel[%d %d %d %d]\n",
			p_det_info->wheel.box[0], p_det_info->wheel.box[1],
			p_det_info->wheel.box[2], p_det_info->wheel.box[3]);
		loc_x0 = p_det_info->wheel.box[0];
        loc_y0 = p_det_info->wheel.box[1];
        loc_x1 = p_det_info->wheel.box[2];
        loc_y1 = p_det_info->wheel.box[3];
		drawframe_info.stRectArray[index].stRectAttr.stRect.s32X = loc_x0;
		drawframe_info.stRectArray[index].stRectAttr.stRect.s32Y = loc_y0;
		drawframe_info.stRectArray[index].stRectAttr.stRect.u32Height = loc_y1-loc_y0;
		drawframe_info.stRectArray[index].stRectAttr.stRect.u32Width = loc_x1-loc_x0;
		drawframe_info.stRectArray[index].stRectAttr.u32BorderSize = 4;
		drawframe_info.stRectArray[index].stRectAttr.u32Color = 0xff00ff00;
		drawframe_info.stRectArray[index].isShow = EI_TRUE;
		drawframe_info.stRgnChnAttr[VENC_RGN_WM_NUM].bShow = EI_TRUE;
		s32Ret = EI_MI_RGN_SetChnAttr(drawframe_info.Handle[VENC_RGN_WM_NUM], &drawframe_info.stRgnChn[1], &drawframe_info.stRgnChnAttr[VENC_RGN_WM_NUM]);
		if(s32Ret) {
	...
	index = index + p_det_info->wheel.exist;
	if (p_det_info->phone.exist) {
		loc_x0 = p_det_info->phone.box[0];
        loc_y0 = p_det_info->phone.box[1];
        loc_x1 = p_det_info->phone.box[2];
        loc_y1 = p_det_info->phone.box[3];
		drawframe_info.stRectArray[index].stRectAttr.stRect.s32X = loc_x0;
		drawframe_info.stRectArray[index].stRectAttr.stRect.s32Y = loc_y0;
		drawframe_info.stRectArray[index].stRectAttr.stRect.u32Height = loc_y1-loc_y0;
		drawframe_info.stRectArray[index].stRectAttr.stRect.u32Width = loc_x1-loc_x0;
		drawframe_info.stRectArray[index].stRectAttr.u32BorderSize = 4;
		drawframe_info.stRectArray[index].stRectAttr.u32Color = 0xff0000ff;
		drawframe_info.stRectArray[index].isShow = EI_TRUE;
		drawframe_info.stRgnChnAttr[VENC_RGN_WM_NUM].bShow = EI_TRUE;
		s32Ret = EI_MI_RGN_SetChnAttr(drawframe_info.Handle[VENC_RGN_WM_NUM], &drawframe_info.stRgnChn[1], &drawframe_info.stRgnChnAttr[VENC_RGN_WM_NUM]);
		if(s32Ret) {
...
int top_dms_warning(top_dms_out_t *data)
{
	int64_t cur_time = osal_get_msec();
	uint32_t index;
	
	EI_PRINT("top dms action belt_state:%d hand_wheel_state:%d play_phone_state:%d\n",
		data->belt_state, data->hand_wheel_state, data->play_phone_state);
	EI_PRINT("top dms warn_status belt_state-%d hand_wheel_state-%d play_phone_state-%d\n",
		data->warn_status.belt_state, data->warn_status.hand_wheel_state, data->warn_status.play_phone_state);
	if (data->warn_status.belt_state) {
		index = LB_WARNING_TOP_DMS_BELT-LB_WARNING_BEGIN-1;
		if ((cur_time - warning_info.last_warn_time[index]) > ALARM_INTERVAL) {
			osal_mq_send(warning_info.mq_id, LB_WARNING_TOP_DMS_BELT, NULL, 0);
			warning_info.last_warn_time[index] = cur_time;
			warning_info.last_warn_status = LB_WARNING_TOP_DMS_BELT;
		}
	}
	if (data->warn_status.hand_wheel_state) {
		index = LB_WARNING_TOP_DMS_WHEEL-LB_WARNING_BEGIN-1;
		if ((cur_time - warning_info.last_warn_time[index]) > ALARM_INTERVAL) {
			osal_mq_send(warning_info.mq_id, LB_WARNING_TOP_DMS_WHEEL, NULL, 0);
			warning_info.last_warn_time[index] = cur_time;
			warning_info.last_warn_status = LB_WARNING_TOP_DMS_WHEEL;
		}
	}
	if (data->warn_status.play_phone_state) {
		index = LB_WARNING_TOP_DMS_PLAY_PHONE-LB_WARNING_BEGIN-1;
		if ((cur_time - warning_info.last_warn_time[index]) > ALARM_INTERVAL) {
			osal_mq_send(warning_info.mq_id, LB_WARNING_TOP_DMS_PLAY_PHONE, NULL, 0);
			warning_info.last_warn_time[index] = cur_time;
			warning_info.last_warn_status = LB_WARNING_TOP_DMS_PLAY_PHONE;
		}
	}
...
static int top_dms_result_cb(int event, void *data)
{

    top_dms_out_t *out = (top_dms_out_t*)data;
	pthread_mutex_lock(&axframe_info.p_det_mutex);
    memcpy(axframe_info.p_det_info, out, sizeof(top_dms_out_t));
	pthread_mutex_unlock(&axframe_info.p_det_mutex);
	if (out->person.exist) {
		EI_PRINT("person[%d %d %d %d]\n",
			out->person.box[0], out->person.box[1],
			out->person.box[2], out->person.box[3]);
	}
	if (out->belt.exist) {
		EI_PRINT("belt[%d %d %d %d]\n",
			out->belt.box[0], out->belt.box[1],
			out->belt.box[2], out->belt.box[3]);
	}
	if (out->wheel.exist) {
		EI_PRINT("wheel[%d %d %d %d]\n",
			out->wheel.box[0], out->wheel.box[1],
			out->wheel.box[2], out->wheel.box[3]);
	}
	if (out->phone.exist) {
		EI_PRINT("phone[%d %d %d %d]\n",
			out->phone.box[0], out->phone.box[1],
			out->phone.box[2], out->phone.box[3]);
	}
	top_dms_warning(out);
...
int top_dms_start(VIDEO_FRAME_INFO_S *frame, void *nn_hdl)
{
    EI_S32 s32Ret;
    static top_dms_in_t m_dms_in;
    ...
    if (nn_hdl != NULL) {
        s32Ret = nn_top_dms_cmd(nn_hdl, NN_TOP_DMS_SET_DATA, &m_dms_in);
    }
        ...
        if (axframe_info.ax_fout) {
                ...
                if (i == 0) {
                    fwrite((void *)frame->stVFrame.ulPlaneVirAddr[i],
                        1,  m_dms_in.img.width*m_dms_in.img.height, axframe_info.ax_fout);
                } else {
                    fwrite((void *)frame->stVFrame.ulPlaneVirAddr[i],
                        1,  m_dms_in.img.width*m_dms_in.img.height/2, axframe_info.ax_fout);
                }
            }
        }
    }
...
EI_VOID *get_axframe_proc(EI_VOID *p)
{
	...
	sprintf(srcfilename, "%s/top_dms_raw_ch%d.yuv", PATH_ROOT, axframe_info->chn);
	...
	int64_t frame_cnt = 0;
#endif
    EI_PRINT("********start_top_dms******");
    memset(&axframe_info->m_top_dms_cfg, 0, sizeof(nn_top_dms_cfg_t));
    /* init top_dms, cfg */
    axframe_info->m_top_dms_cfg.img_width = axframe_info->u32Width; //Ëã·¨ÊäÈëÍ¼Ïñ¿í
    axframe_info->m_top_dms_cfg.img_height = axframe_info->u32Height; //Ëã·¨ÊäÈëÍ¼Ïñ¸ß
    axframe_info->m_top_dms_cfg.format = YUV420_SP;
    axframe_info->m_top_dms_cfg.model_basedir = TOP_DMS_PATH;
    axframe_info->m_top_dms_cfg.cb_func = top_dms_result_cb;
	axframe_info->m_top_dms_cfg.det_en = 1; /* enable human body detect */
	axframe_info->m_top_dms_cfg.classfication_en = 1; /* enable object classfication */

    axframe_info->m_top_dms_cfg.interest_box[0] = 0;//Ëã·¨¼ì²âÇøÓò×óÉÏ½ÇX×ø±ê
    axframe_info->m_top_dms_cfg.interest_box[1] = 0;//Ëã·¨¼ì²âÇøÓò×óÉÏ½Çy×ø±ê
    axframe_info->m_top_dms_cfg.interest_box[2] = axframe_info->u32Width - 1;//Ëã·¨¼ì²âÇøÓòÓÒÏÂ½Çx×ø±ê
    axframe_info->m_top_dms_cfg.interest_box[3] = axframe_info->u32Height - 1;//Ëã·¨¼ì²âÇøÓòÓÒÏÂ½Çy×ø±ê
    EI_PRINT("interest_box x1:%d,y1:%d,x2:%d,y2:%d \n", axframe_info->m_top_dms_cfg.interest_box[0], axframe_info->m_top_dms_cfg.interest_box[1],
        axframe_info->m_top_dms_cfg.interest_box[2], axframe_info->m_top_dms_cfg.interest_box[3]);
	axframe_info->m_top_dms_cfg.confidence_threshold[0] = 0.5; /* person  model confidence thresh, rang 0.35-0.9 */
	axframe_info->m_top_dms_cfg.confidence_threshold[1] = 0.65; /* phone  model confidence thresh, rang 0.4-0.9 */
	axframe_info->m_top_dms_cfg.confidence_threshold[2] = 0.6; /* wheel  model confidence thresh, rang 0.35-0.9 */
	axframe_info->m_top_dms_cfg.confidence_threshold[3] = 0.45; /* belt  model confidence thresh, rang 0.35-0.85 */
	axframe_info->m_top_dms_cfg.confidence_threshold[4] = 0.5; /* hand  model confidence thresh, rang 0.35-0.9 */
    axframe_info->m_top_dms_cfg.warn_cfg.no_hand_time = 5000;/* 5000ms */
    axframe_info->m_top_dms_cfg.warn_cfg.no_belt_time = 10000;/* 10000ms */
    axframe_info->m_top_dms_cfg.warn_cfg.play_phone_time = 3000;/* 3000ms */
    /* open libtopdms.so */
    nn_hdl = nn_top_dms_open(&axframe_info->m_top_dms_cfg);
    if (nn_hdl == NULL) {
        EI_PRINT("top_dms_open() failed!");
        return NULL;
    }
#ifdef AL_PRO_FRAMERATE_STATISTIC
		time1 = osal_get_msec();
...
EI_VOID *get_drawframe_proc(EI_VOID *p)
{
    ...
    while (EI_TRUE == drawframe_info->bThreadStart) {
		...
		pthread_mutex_lock(&axframe_info.p_det_mutex);
		top_dms_draw_location_rgn(&drawFrame, axframe_info.p_det_info);
		pthread_mutex_unlock(&axframe_info.p_det_mutex);
...
EI_VOID *warning_proc(EI_VOID *p)
{
	...
	while (warning_info->bThreadStart) {
		...
		EI_PRINT("msg.type = %d\n", msg.type);
		switch (msg.type) {
			case LB_WARNING_TOP_DMS_NO_FACE:
				warn_tone_play("/usr/share/out/res/top_dms_warning/weijiancedaojiashiyuan.wav");
				break;
			case LB_WARNING_TOP_DMS_BELT:
				warn_tone_play("/usr/share/out/res/top_dms_warning/qingjianquandai.wav");
				break;
			case LB_WARNING_TOP_DMS_WHEEL:
				warn_tone_play("/usr/share/out/res/top_dms_warning/qingwojinfangxiangpan.wav");
				break;
			case LB_WARNING_TOP_DMS_PLAY_PHONE:
				warn_tone_play("/usr/share/out/res/top_dms_warning/qingbuyaowanshouji.wav");
				break;
			default:
				break;
		}
    ...
    SNS_TYPE_E sns)
{
    ...
    axframe_info.p_det_info = malloc(sizeof(top_dms_out_t));
    if (!axframe_info.p_det_info) {
		printf("axframe_info.p_det_info malloc failed!\n");
		goto exit1;
    }

    memset(axframe_info.p_det_info, 0x00, sizeof(top_dms_out_t));
