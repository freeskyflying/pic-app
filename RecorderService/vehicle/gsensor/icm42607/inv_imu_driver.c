...
//	inv_imu_sleep_us(3000);
	...
	s->gyro_start_time_us = 1000;//UINT32_MAX;
	s->accel_start_time_us = 1000;//UINT32_MAX;
	s->gyro_power_off_tmst = 1000;//UINT32_MAX;
...
int inv_imu_get_data_from_fifo(struct inv_imu_device *s)
{
	...
	if ((status |= inv_imu_read_reg(s, INT_STATUS, 1, &int_status)))
		...
		//INV_MSG(INV_MSG_LEVEL_ERROR, "status %", int_status);
