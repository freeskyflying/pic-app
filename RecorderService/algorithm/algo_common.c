...
void Algo_Start(void)
{
	...
	{
		adas_nn_init(0/*ippu dev*/, 0, 0/*ippu chn*/, 1920, 1080);
	}
#endif
}
...
int al_warning_proc(WARNING_MSG_TYPE warnType, int extra, int extra2, float ttc)
{
	...
	extern int JT808ActiveSafety_ProcAlarm(int type, int extra, int extra2, float ttc, int speed);
	
	ret |= JT808ActiveSafety_ProcAlarm(warnType, extra, extra2, ttc, gAlComParam.mCurrentSpeed);

	extern int Mqtt_Ftp_Alarm(int type, int extra, int extra2, int speed);
	ret |= Mqtt_Ftp_Alarm(warnType, extra, extra2, gAlComParam.mCurrentSpeed);
