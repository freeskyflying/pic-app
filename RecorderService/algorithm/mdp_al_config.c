...
int adas_save_cali_parm(float vanish_point_x, float vanish_point_y, float bottom_line_y)
{
	...
	if (al_config_obj.vanish_point_x == NULL) return -1;
	if (al_config_obj.vanish_point_y == NULL) return -1;
	if (al_config_obj.bottom_line_y == NULL) return -1;
...
int dms_save_cali_parm(float yaw, float pitch)
{
	...
	if (al_config_obj.yaw == NULL) return -1;
	if (al_config_obj.pitch == NULL) return -1;
