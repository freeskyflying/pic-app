...
retry_ones:
	...
	SAMPLE_COMM_VENC_Stop(handle->vencChn);

	PNT_LOGE("task %s exit.", prname);
...
int MediaSnap_Init(void)
{
	...
	int encChn = MAX_VIDEO_NUM*2, jvcChn = 0;
	
#ifdef DISABLECH1
#else
	SAMPLE_VENC_CONFIG_S* pStSnapJVC = &stSnapJVC[jvcChn];
    pStSnapJVC->enInputFormat   = PIX_FMT_YUV_SEMIPLANAR_420;
    pStSnapJVC->u32width        = 1920;
    pStSnapJVC->u32height       = 1080;
    pStSnapJVC->u32bitrate      = 30*1024*1024;
	
	if (gGlobalSysParam->mirror[0])
	{
		pStSnapJVC->enMirrorDirection = MIRDIRDIR_HOR;
		pStSnapJVC->enRotation = ROTATION_0;
	}
	else
	{
		pStSnapJVC->enMirrorDirection = MIRDIRDIR_NONE;
		pStSnapJVC->enRotation = ROTATION_0;
	}

    ret = SAMPLE_COMM_VENC_Start(encChn, PT_JPEG, SAMPLE_RC_FIXQP, pStSnapJVC, COMPRESS_MODE_NONE, VENC_GOPMODE_DUAL_P);
    if (EI_SUCCESS != ret)
	{
    ...
    EI_MI_VENC_GetJpegParam(encChn, &stJpegParam);
    stJpegParam.bEnableAreaCode = 1;
    EI_MI_VENC_SetJpegParam(encChn, &stJpegParam);
	
	gSnapController[jvcChn].vencChn = encChn;
	gSnapController[jvcChn].vissDev = 0; // IPPU dev 0
	gSnapController[jvcChn].vissChn = 0; // IPPU chn 0
	gSnapController[jvcChn].width = 1920;
	gSnapController[jvcChn].height = 1080;
	encChn ++;
	jvcChn ++;
#endif
...
#ifdef DISABLECH2
#else
	pStSnapJVC = &stSnapJVC[jvcChn];
	pStSnapJVC->enInputFormat	 = PIX_FMT_YUV_SEMIPLANAR_420;
	pStSnapJVC->u32width		 = 1280;
	pStSnapJVC->u32height		 = 720;
	pStSnapJVC->u32bitrate 	 = 10*1024*1024;
	
	if (gGlobalSysParam->mirror[1])
	{
		pStSnapJVC->enMirrorDirection = MIRDIRDIR_HOR;
		pStSnapJVC->enRotation = ROTATION_0;
	}
	...
	gSnapController[jvcChn].vencChn = encChn;
	gSnapController[jvcChn].vissDev = 2;
	gSnapController[jvcChn].vissChn = 1;
	gSnapController[jvcChn].width = 1280;
	gSnapController[jvcChn].height = 720;
	encChn ++;
	jvcChn ++;
#endif
	...
	gSnapController[jvcChn].vencChn = encChn;
	gSnapController[jvcChn].vissDev = 2;
	gSnapController[jvcChn].vissChn = 2;
	gSnapController[jvcChn].width = 1280;
	gSnapController[jvcChn].height = 720;
	encChn ++;
	jvcChn ++;
...
void MediaSnap_Stop(void)
{
	for (int i=0; i<sizeof(gSnapController)/sizeof(gSnapController[0]); i++)
	{
		gSnapController[i].bThreadFlag = 0;
	}
	usleep(200*1000);
}
