...
static EI_S32 MediaOsd_Update2REG(uint8_t* pbuf, EI_U32 u32Width, EI_U32 u32Height, RGN_HANDLE handleId)
{
	...
	uint8_t *pOrigbuf = (uint8_t*)stCanvasInfo.pVirtAddr;
    memset(pOrigbuf,0,height*width);
	
	if (gGlobalSysParam->mirror[handleId])
	{		
		for (int i=0; i<h1; i++)
		{
			x = width - 1;
			...
			for (int j=0; j<w1; j++)
			{
				*(pOrigbuf + ((y+i)*width + x - j)*2) = *(pbuf + i*stride + j*2);
				*(pOrigbuf + ((y+i)*width + x - j)*2 + 1) = *(pbuf + i*stride + j*2 + 1);
				//memcpy(pOrigbuf + ((y+i)*width + x)*2, pbuf + i*stride + j*2, 2);
			}
		}
	}
	else
...
static int MediaOsd_UpdateOsd(int chn, OsdInfo_t* osdInfo)
{
    ...
    else if (RES_CIF == gMediaOsdPara.res[chn])
    {
        buffwidth = 640;
    }
...
static int constructOsdChnTitle(MediaOsdPara_t *para)
{
    ...
    {    
        ...
        {
        	if (gGlobalSysParam->mSpdOsdType)
        	{
        		float speed = (float)para->speed/1.61;
            	sprintf((char*)info.text,"%s %dmil/h",(char*)info.text,(uint16_t)speed);
        	}
			else
...
static int LocationIPCIRcvDataProcess(char* buff, int len, int fd, void* arg)
{
    ...
    if (len >= sizeof(GpsLocation_t))
    {
        ...
        gMediaOsdPara.speed = location.speed;
		gAlComParam.mCurrentSpeed = location.speed;
		gAlComParam.mGpsValid = location.valid;
...
EI_S32 MediaOsd_InitChn(MDP_CHN_S *stRgnChn, EI_U32 chnWidth, EI_U32 chnHeight, EI_U32 handleId)
{
    ...
    else if (1280 == chnWidth || 960 == chnWidth)
    {
    	...
    	stRgnChnAttr.unChnAttr.stOverlayExChn.stPoint.s32Y = 10;
    }
    else
    {
        gMediaOsdPara.res[handleId] = RES_CIF;
   	 	stRgnChnAttr.unChnAttr.stOverlayExChn.stPoint.s32Y = 10;
    }
