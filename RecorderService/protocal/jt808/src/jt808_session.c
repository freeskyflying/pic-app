...
int TTSMainCompleteNotify(char* path)
{
	...
	sprintf(cmd, "aplay -r 8000 -c 1 -f S16_LE %s", path);
...
void JT808FtpUpgradeSaveSerialNum(WORD serialNum)
{
	FILE* pf = fopen("/data/FtpUpgrade.ack", "wb");
	if (pf)
	{
		fwrite(&serialNum, 1, sizeof(serialNum), pf);
		fclose(pf);
	}
}

void JT808FtpUpgradeResultAck(JT808Session_t* session, int result)
{
	JT808MsgBody_0001_t ack = { 0 };
	FILE* pf = fopen("/data/FtpUpgrade.ack", "rb");
	if (pf)
	{
		fread(&ack.mResponseSerialNum, 1, sizeof(ack.mResponseSerialNum), pf);
		fclose(pf);
		remove("/data/FtpUpgrade.ack");
	}
//	ack.mMsgID = 0x8105;
//	ack.mResult = result;
//	JT808Session_SendMsg(session, 0x0001, &ack, sizeof(ack), 0);

//	if (COM_RESULT_SUCCESS == result)
	{
		JT808MsgBody_0108_t ack0108 = { 0 };
		ack0108.mUpgradeResult = result;
		ack0108.mUpgradeType = 0;
		
		JT808Session_SendMsg(session, 0x0108, &ack0108, sizeof(ack0108), 0);
	}
}
...
void JT808FtpUpgradeDownloadComplete(void* arg)
{
	...
	JT808Session_t* session = pFtpUtil->mOwner;
	...
	PNT_LOGI("md5sum [%s]: %s.", originFullPath, md5sum);

	if (0 == strlen(md5sum) || NULL == strstr(pFtpUtil->mFileName, md5sum)) // 文件名包含md5参数
	{
		JT808FtpUpgradeResultAck(session, COM_RESULT_FAILED);
	...
	remove(JT808_FTP_DOWNLOAD_URL);
	session->mFtpDownloadFlag = 0;
}
...
void JT808FtpUpgradeDownloadFailed(void* arg)
{
	...
	if (CURLE_REMOTE_FILE_NOT_FOUND == pFtpUtil->result ||
		CURLE_TFTP_NOSUCHUSER == pFtpUtil->result ||
		CURLE_TFTP_NOTFOUND == pFtpUtil->result ||
		CURLE_LOGIN_DENIED == pFtpUtil->result)
	{
		PNT_LOGE("fatal error, delete jt808 downlowd task.");
		remove(JT808_FTP_DOWNLOAD_URL);
	}
...
int JT808FtpUpgradeProc(char* str, JT808Session_t* session)
{
	FtpUtil_t ftpUtil = { 0 };
	memset(&ftpUtil, 0, sizeof(ftpUtil));

	if (session->mFtpDownloadFlag)
	{
		return RET_FAILED;
	}
	...
	clearTargetDirExt(JT808_FTP_UPGRADE_TMP_PATH, ftpUtil.mFileName);

	return FtpUtil_DownloadCreate(&ftpUtil);
}

void JT808Session_GetTerminalAttribute(JT808Session_t* session, JT808MsgBody_0107_t* ack)
{
	JT808Controller_t* controller = (JT808Controller_t*)session->mOnwer;

	memset(ack, 0, sizeof(JT808MsgBody_0107_t));

	ack->mTerminalType = (1<<0) | (1<<3);
	
	memcpy(ack->mProducerId, "44030", sizeof(ack->mProducerId));
	
	strcpy((char*)ack->mTerminalModel, gBoardInfo.mProductModel);
	
	memcpy(ack->mTerminalId, controller->mDeviceID, sizeof(controller->mDeviceID));

	memcpy(ack->mTerminalSIMICCIDCode, gNet4GRild.CCID, sizeof(ack->mTerminalSIMICCIDCode));

	strcpy(ack->mTerminalHardwareVersion, gBoardInfo.mHwVersion);
	ack->mTerminalHardwareVersionLen = strlen(ack->mTerminalHardwareVersion);
	
	strcpy(ack->mTerminalFirmwareVersion, gBoardInfo.mSwVersion);
	ack->mTerminalFirmwareVersionLen = strlen(ack->mTerminalFirmwareVersion);

	ack->mGNSSModuleAttribute = (1<<0) | (1<<1);
	ack->mCommunicationModuleAttibute = (1<<0) | (1<<1) | (1<<2) | (1<<3) | (1<<4) | (1<<6);
}
...
int JT808Session_AddWaitAckMsg(JT808Session_t* session, char* dataBuf, int dataLen, WORD msgId, WORD msgSerialNum, char* bcdtime)
{
	...
	waitAckNode->mLastTimeStamp = (uint32_t)currentTimeMillis()/1000;
...
int JT808Session_DeleteWaitAckMsg(JT808Session_t* session, WORD msgId, WORD msgSerialNum)
{
	...
	while (head != NULL)
	{
		...
		if (msgId == node->mMsgId && (msgSerialNum == 0x0000 || msgSerialNum == node->mMsgSerilNum))
		{
			...
			{
				if (controller->mCurrentAlertFlag &JT808_ALERT_FLAG_SOS)
				{
					//controller->mCurrentAlertFlag &= (~JT808_ALERT_FLAG_SOS);
				}
				if (controller->mCurrentAlertFlag & JT808_ALERT_FLAG_GETINOUTAREA)
				{
					controller->mCurrentAlertFlag &= (~JT808_ALERT_FLAG_GETINOUTAREA);
				}
				if (controller->mCurrentAlertFlag & JT808_ALERT_FLAG_GETINOUTROAD)
				{
...
void* JT808Session_QueryFilesTask(void* param)
{
    pthread_detach(pthread_self());

	JT808Session_t* session = (JT808Session_t*)param;
	...
	ack.mReqSerialNum = session->mMediaQuerySerialNum;
	
	JT1078MediaFiles_QueryFiles(session->mMediaQueryParam, &ack);
	...
	free(session->mMediaQueryParam);
	session->mMediaQueryParam = NULL;
...
int JT808Session_RcvMsgProcess(JT808Session_t* session, JT808MsgHeader_t* msgHead, void* msgBody)
{
	...
	switch (msgHead->mMsgID)
	{
					...
					case 0x0102:
						{
							if (0 == param->mResult)
							{
								...
								if (lastResult > 0)
								{
									PNT_LOGE("last upgrade result : %d.", lastResult);
									
									if (2 == lastResult)
									{
										JT808FtpUpgradeResultAck(session, COM_RESULT_FAILED);
									}
									else if (3 == lastResult)
									{
										JT808FtpUpgradeResultAck(session, COM_RESULT_SUCCESS);
									}
									remove(UPGRADE_FLAG_RESULT_FLAG);
								}

								if (0 == session->mFtpDownloadFlag)
								{
									char url[128] = { 0 };
									if (0 == getStringByFile(JT808_FTP_DOWNLOAD_URL, url, sizeof(url)))
									{
										PNT_LOGI("re-download jt808 upgrade file %s.", url);
										if (0 == JT808FtpUpgradeProc(url, session))
										{
											session->mFtpDownloadFlag = 1;
										}
									}
								}
		...
		case 0x8103:	// 设置终端参数
			{
				...
				for (int i=0; i<param->mParamItemCount; i++)
				{
					...
					if (0xF364 == item->mParamID || 0xF365 == item->mParamID || 0xF366 == item->mParamID || 0xF367 == item->mParamID)
					{
						JT808ActiveSafety_UpdateParamsFromMem(item->mParamID, (unsigned char*)item->mParamData, 1);
					}
					if (0x55 == item->mParamID)
					{
						gGlobalSysParam->limitSpeed = item->mParamData;
					}

					JT808Overspeed_ParamUpdate(&controller->mOverspeed, item->mParamID);
		...
		case 0x8107:
			{
				JT808MsgBody_0107_t paramAck = { 0 };
				JT808Session_GetTerminalAttribute(session, &paramAck);
				JT808Session_SendMsg(session, 0x0107, &paramAck, sizeof(paramAck), 0);
			}
			break;
		...
		case 0x8202:
			{
				...
				JT808MsgBody_8202_t* param = (JT808MsgBody_8202_t*)msgBody;
				session->mTempReportIntervel = param->mTimeIntervel;
				session->mTempReportValidTime = param->mValidDate;
				session->mTempReportStartTime = currentTimeMillis()/1000;
			}
			break;
		...
		case 0x8300:
			{
				...
				if ((param->mTextFlag & 0x08) && strlen(param->mTextContent) > 0)
				{
					str_normalize_init();
		...
		case 0x9003:	// 查询终端音视频属性
			{
				...
				JT1078Rtsp_GetMediaAttribute(&ack);
		...
		case 0x8105:
			{
				...
				switch(param->mTerminalCtrlCmd)
				{
					case 1:
						{
							...
							if (JT808FtpUpgradeProc(param->pTerminalCtrlParam, session))
							{
								ack.mResult = COM_RESULT_FAILED;
							}
							else
							{
								session->mFtpDownloadFlag = 1;
								JT808FtpUpgradeSaveSerialNum(msgHead->mMsgSerialNum);
								setStringToFile(JT808_FTP_DOWNLOAD_URL, param->pTerminalCtrlParam);
							}
						}
						break;
					case 2:
					...
					case 4:
						{
							JT808Session_SendMsg(session, 0x0001, &ack, sizeof(ack), 0);
							extern void protocal_exit();
							extern void MediaStorage_Exit(void);
							extern int MediaVideo_Stop(void);
							protocal_exit();
							MediaStorage_Exit();
							MediaVideo_Stop();
							system("reboot -f");
				...
				if (0)//(param->mTerminalCtrlCmd == 1 && ack.mResult == COM_RESULT_SUCCESS)
				{
				}
				else
				{
		...
		case 0x9205:
			{
				...
				if (NULL == session->mMediaQueryParam)
				{
					session->mMediaQueryParam = (JT808MsgBody_9205_t*)malloc(sizeof(JT808MsgBody_9205_t));
					if (NULL != session->mMediaQueryParam)
					{
						memcpy(session->mMediaQueryParam, param, sizeof(JT808MsgBody_9205_t));
						
						pthread_t pid;
						session->mMediaQuerySerialNum = msgHead->mMsgSerialNum;
						pthread_create(&pid, NULL, JT808Session_QueryFilesTask, session);
					}
				}
			}
			break;
		...
		case 0x9201:
			{
				...
				JT1078MediaFiles_PlaybackRequest(session, param, &ack);
		...
		case 0x9202:
			{
				...
				JT1078MediaFiles_PlaybackControl(session, param);
		...
		case 0x8600:
			{
				...
				JT808MsgBody_8600_t* param = (JT808MsgBody_8600_t*)msgBody;
				if (0 == param->mSettingOption)
				{
					JT808CircleArea_DeleteAllItem(&controller->mCircleArea);
				}
				for (int i=0; i<param->mAreaCount; i++)
				{
					JT808CircleArea_AddItem(&controller->mCircleArea, &param->pCircleAreaItems[i]);
				}
				if (param->mAreaCount > 0)
				{
					free(param->pCircleAreaItems);
					param->pCircleAreaItems = NULL;
				}
			}
			break;
		...
		case 0x8601:
			{
				...
				JT808MsgBody_8601_t* param = (JT808MsgBody_8601_t*)msgBody;
				for (int i=0; i<param->mDeleteAreaCount; i++)
				{
					JT808CircleArea_DeleteItem(&controller->mCircleArea, param->mDeleteAreaIDs[i]);
				}
				if (0 == param->mDeleteAreaCount)
				{
					JT808CircleArea_DeleteAllItem(&controller->mCircleArea);
				}
			}
		...
		case 0x8602:
			{
				...
				JT808MsgBody_8602_t* param = (JT808MsgBody_8602_t*)msgBody;
				if (0 == param->mSettingOption)
				{
					JT808RectArea_DeleteAllItem(&controller->mRectArea);
				}
				for (int i=0; i<param->mAreaCount; i++)
				{
					JT808RectArea_AddItem(&controller->mRectArea, &param->pRectangleAreaItems[i]);
				}
				if (param->mAreaCount > 0)
				{
					free(param->pRectangleAreaItems);
					param->pRectangleAreaItems = NULL;
				}
			}
			break;
		...
		case 0x8603:
			{
				...
				JT808MsgBody_8603_t* param = (JT808MsgBody_8603_t*)msgBody;
				for (int i=0; i<param->mDeleteAreaCount; i++)
				{
					JT808RectArea_DeleteItem(&controller->mRectArea, param->mDeleteAreaIDs[i]);
				}
				if (0 == param->mDeleteAreaCount)
				{
					JT808RectArea_DeleteAllItem(&controller->mRectArea);
		...
		case 0x8604:
			{
				...
				JT808MsgBody_8604_t* param = (JT808MsgBody_8604_t*)msgBody;

				JT808PolygonArea_AddItem(&controller->mPolygonArea, param);
		...
		case 0x8605:
			{
				...
				JT808MsgBody_8605_t* param = (JT808MsgBody_8605_t*)msgBody;
				for (int i=0; i<param->mDeleteAreaCount; i++)
				{
					JT808PolygonArea_DeleteItem(&controller->mPolygonArea, param->mDeleteAreaIDs[i]);
				}
				if (0 == param->mDeleteAreaCount)
				{
					JT808PolygonArea_DeleteAllItem(&controller->mPolygonArea);
				}
			}
			break;
		...
		case 0x8606:
			{
				...
				JT808MsgBody_8606_t* param = (JT808MsgBody_8606_t*)msgBody;

				JT808RoadArea_AddItem(&controller->mRoadArea, param);

				if (param->mInflectionPointCount > 0)
				{
					free(param->pInflectionPoints);
					param->pInflectionPoints = NULL;
				}
			}
			break;
		...
		case 0x8607:
			{
				...
				JT808MsgBody_8607_t* param = (JT808MsgBody_8607_t*)msgBody;
				for (int i=0; i<param->mDeleteRouteCount; i++)
				{
					JT808RoadArea_DeleteItem(&controller->mRoadArea, param->mDeleteRouteIDs[i]);
				}
				if (0 == param->mDeleteRouteCount)
				{
					JT808RoadArea_DeleteAllItem(&controller->mRoadArea);
				}
			}
			break;
		...
		case 0x8608:
			{
				JT808MsgBody_8608_t* param = (JT808MsgBody_8608_t*)msgBody;
				BYTE* ack = NULL;
				int ackLen = JT808Area_Query(controller, param, &ack);
				if (0 < ackLen && NULL != ack)
				{
					JT808Session_SendMsg(session, 0x0608, ack, ackLen, 1);
					free(ack);
					ack = NULL;
				}
				if (param->mQueryIDCount)
				{
					free(param->pQueryIDs);
					param->pQueryIDs = NULL;
				}
			}
			break;
		...
		case 0x8108:
			{
				JT808MsgBody_0108_t ack = { 0 };

				JT808MsgBody_8108_t* param = (JT808MsgBody_8108_t*)msgBody;
				if (TRUE == ensureTargetDirExist(TERMINAL_OTA_PATH))
				{
					char filename[128] = { 0 };
					sprintf(filename, JT808_FILE_UPGRADE_FILENAME, gBoardInfo.mProductModel);
					FILE* pFile = fopen(filename, "wb");
					if (NULL == pFile)
					{
						ack.mUpgradeResult = 1;
					}
					else
					{
						fwrite(param->pTerminalFirmwareData, 1, param->mTerminalFirmwareDataLen, pFile);
						fclose(pFile);
						ack.mUpgradeResult = 0;
				...
				ack.mUpgradeType = param->mUpgradeType;
				JT808Session_SendMsg(session, 0x0108, &ack, sizeof(ack), 0);
			}
			break;
		...
		case 0x8203:
			{
				...
				JT808MsgBody_8203_t* param = (JT808MsgBody_8203_t*)msgBody;
//				if (0 == param->mAlertMsgSerialNumber)
				{
					if (JT808_ALERT_MANMADE_TYPE_SOS & param->mAlertType)
					{
						if (controller->mCurrentAlertFlag &JT808_ALERT_FLAG_SOS)
						{
							controller->mCurrentAlertFlag &= (~JT808_ALERT_FLAG_SOS);
					...
					if (JT808_ALERT_MANMADE_TYPE_PRE_DANGER & param->mAlertType)
					{
					}
					if (JT808_ALERT_MANMADE_TYPE_INOUTAREA & param->mAlertType)
					{
					}
					if (JT808_ALERT_MANMADE_TYPE_INOUTROAD & param->mAlertType)
					{
					}
					if (JT808_ALERT_MANMADE_TYPE_ROADFASTORSLOW & param->mAlertType)
					{
					}
					if (JT808_ALERT_MANMADE_TYPE_ACCILLEGAL & param->mAlertType)
					{
					}
					if (JT808_ALERT_MANMADE_TYPE_MOVINGILLEGAL & param->mAlertType)
					{
					}
				}
			}
		...
		case 0x8800:
			{
				...
				JT808MsgBody_8800_t* param = (JT808MsgBody_8800_t*)msgBody;
				JT808MediaEvent_UploadMediaFileStatus(&session->mMediaEvent, param);
			}
			break;

		case 0x8801:
			{
				JT808MsgBody_0805_t ack = { 0 };
				
				JT808MsgBody_8801_t* param = (JT808MsgBody_8801_t*)msgBody;
				if (param->mCameraChannelID > MAX_VIDEO_NUM || 0 == param->mCaptureIntervel || 0xFFFF == param->mCaptureCommand)
				{
					ack.mResult = 2;
				}
				else
				{
					JT808MediaEvent_MediaPhotoEvent(&session->mMediaEvent, JT808MediaEventType_PlatCmd, param->mCameraChannelID, 
						param->mCaptureCommand, param->mStoreFlag, param->mCaptureIntervel, param->mResolution);
				}
				ack.mResponseSerialNum = msgHead->mMsgSerialNum;
				JT808Session_SendMsg(session, 0x0805, &ack, sizeof(ack), 0);
			}
			break;

		case 0x8802:
			{
				JT808MsgBody_0802_t ack = { 0 };

				ack.mResponseSerialNum = msgHead->mMsgSerialNum;
				
				JT808MsgBody_8802_t* param = (JT808MsgBody_8802_t*)msgBody;

				JT808MediaEvent_QueryMediaFiles(&session->mMediaEvent, param, &ack);

				if (NULL != ack.pItemDatas)
				{
					int bodyLen = ack.mDataLen + 4;
					BYTE* tmp = (BYTE*)malloc(bodyLen);
					if (NULL != tmp)
					{
						bodyLen = JT808_0802_CmdEncoder(&ack, tmp, bodyLen, session);
						JT808Session_SendMsg(session, 0x0802, tmp, bodyLen, 1);
						free(tmp);
						tmp = NULL;
					}
					free(ack.pItemDatas);
				...
				{
					ack.mItemsCount = 0;
					JT808Session_SendMsg(session, 0x0802, &ack, sizeof(ack), 0);
				}
			}
			break;
		...
		case 0x8803:
			{
				...
				JT808MsgBody_8803_t* param = (JT808MsgBody_8803_t*)msgBody;

				JT808MediaEvent_UploadMediaFilesReq(&session->mMediaEvent, param);
			}
			break;
		...
		case 0x8805:
			{
				...
				JT808MsgBody_8805_t* param = (JT808MsgBody_8805_t*)msgBody;
				
				JT808MediaEvent_UploadMediaFileReq(&session->mMediaEvent, param);
			}
			break;
...
int JT808Session_MsgDataParser(JT808Session_t* session, char* data, WORD len)
{
	...
	else
	{
		if (JT808MSG_ATTR_IS_PACK(head->mMsgAttribute)) // 分包消息
		{
			...
			if (NULL == node)
			{
				node = (JT808MsgRecvPacketNode_t*)malloc(sizeof(JT808MsgRecvPacketNode_t));
				memset(node, 0, sizeof(JT808MsgRecvPacketNode_t));
				node->mMsgHead = *head;
				node->mPackIdxList = (BYTE*)malloc(node->mMsgHead.mPacketTotalNum);
				node->mLastTimeStamp = currentTimeMillis()/1000;
				...
				memset(node->mMsgBody, 0, node->mMsgHead.mPacketTotalNum*(1024+4));
			...
			if (NULL != node)
			{
				node->mMsgHead.mPacketIndex = head->mPacketIndex;
				...
				memcpy(node->mMsgBody+offsetBody+4, data+offset, bodylen);
		...
		else
		{
			...
			if (NULL == node->mMsgBody)
			{
	            JT808SESSION_PRINT(session, "malloc failed [%04X].", head->mMsgID);
				free(head);
				head = NULL;
			}
			else
    ...
    for (i = 0; i<len; i++)
    {
        if (JT808_MSG_FLAG == buff[i])
        {
            ...
            else
            {
	            if ((i - start_pos - 1) > 10)
	            {
					PRINTF_BUFFER(session, (BYTE*)(buff+start_pos), i - start_pos + 1);
...
void JT808Session_RecvPacketQueueProcess(JT808Session_t* session)
{
	...
	while (head != NULL)
	{
		...
		if (node->mFinishFlag || ((currentTimeMillis()/1000)-node->mLastTimeStamp) > 10)/*10s内未收到新的包*/
		{			
			...
			if (complete_flag)
			{
				...
				if (nodet)
				{
					...
					if (nodet->mMsgHead)
					{
						...
						if (NULL == node->mMsgBody)
						{
				            JT808SESSION_PRINT(session, "malloc failed [%04X].", node->mMsgHead.mMsgID);
							free(nodet->mMsgHead);
							free(nodet);
						}
						else
						{
							...
							for (int i=0; i<node->mMsgHead.mPacketTotalNum; i++)
							{
								int bodyOffset = i*(1024+4);
								
								memcpy(&len, node->mMsgBody + bodyOffset, 4);
								if (len >= 1024)
								{
									PNT_LOGE("len error [%d, %d] [%d].", i, len, node->mMsgBodyLen);
									len = 1024;
								}
								if (offset + len > node->mMsgBodyLen)
								{
									PNT_LOGE("data error.");
								}
								else
								{
									memcpy(nodet->mMsgBody+offset, node->mMsgBody + bodyOffset + 4, len);
									offset += len;
								}
							}
							
							memcpy(nodet->mMsgHead, &node->mMsgHead, sizeof(JT808MsgNode_t));
			...
			{
				if (SESSION_TYPE_JT808_2019 == session->mSessionData->mSessionType)  // 需要补传
				{
...
static void insert0200BlindData(JT808Session_t* session, unsigned char* msgData, int msgLen, unsigned short msgSerialNum)
{
	char msgOrg[2048] = { 0 };
	...
	int orgLen = JT808MsgConvert((char*)(msgData+1), msgLen-2, msgOrg, sizeof(msgOrg), 0);

	if (!(msgOrg[bodyOffset+7] & JT808_TERMINAL_STATE_GPS_STATE))
	{
		return; // 未定位数据不保存
	}
	
	// 保存盲区数据
	JT808BlindUpload_InsertLocationData(&session->mBlineGpsData, msgSerialNum, orgLen-bodyOffset-1, (BYTE*)(msgOrg+bodyOffset));
}
...
void* JT808Session_SendQueueProcess(void* arg)
{
    ...
    while (session->mRunningFlag)
    {
        ...
        else
        {
			...
			if (0x003 == node->mMsgHead->mMsgID)
			{
				session->mRunningFlag = 0;
			}
				...
				for (i = 0; i<sizeof(JT808CMD_SUPPORT_LIST)/sizeof(JT808CMD_SUPPORT_LIST[0]); i++)
				{
					if (node->mMsgHead->mMsgID == JT808CMD_SUPPORT_LIST[i].mMsgID)
					{
						if (CMD_TYPE_INITIATE == JT808CMD_SUPPORT_LIST[i].mType /* && 重传次数>0 */) // 主动发送的消息，需要入等待应答队列
						{
							...
							if (session->mMainProcessState == JT808SessionState_Offline ||
								session->mMainProcessState == JT808SessionState_Waiting)
							{
								if (0x0200 == node->mMsgHead->mMsgID)
								{
									insert0200BlindData(session, sendbuf, sendLen+2, node->mMsgHead->mMsgSerialNum);
								}
							}
							else
							{
...
void* JT808Session_WaitAckMsgQueueProcess(void* arg)
{
	...
	while (session->mRunningFlag)
	{
		...
		while (head != NULL)
		{
			...
			if (((uint32_t)currentTimeMillis()/1000 - node->mLastTimeStamp) > (uint32_t)(node->mTimeoutValue))
			{
				if (node->mReplyCount >= 2) // 设置参数，TCP_MSG_RETRY_COUNT
				{
					...
					if (0x0200 == node->mMsgId)
					{
						insert0200BlindData(session, node->mMsgData, node->mMsgDataLen, node->mMsgSerilNum);
					}
				...
				else
				{
					...
					node->mLastTimeStamp = (uint32_t)currentTimeMillis()/1000;
...
void* JT808Session_LocationMsgReportProcess(void* arg)
{
	...
	DWORD* Cmd0200ReportTimeIntervalEmergency = (DWORD*)JT808Database_GetJT808Param(&controller->mJT808DB, 0x0028, NULL);
	DWORD* Cmd0200ReportDistIntervalEmergency = (DWORD*)JT808Database_GetJT808Param(&controller->mJT808DB, 0x002F, NULL);
	...
	while (session->mRunningFlag)
	{
		...
		if (controller->mCurrentAlertFlag & JT808_ALERT_FLAG_SOS)
		{
			if (NULL != Cmd0200ReportTimeIntervalEmergency)
			{
				reportTimeInterval = *Cmd0200ReportTimeIntervalEmergency;
			}
			if (NULL != Cmd0200ReportDistIntervalEmergency)
			{
				reportDistInterval = *Cmd0200ReportDistIntervalEmergency;
			}
		}
		
		if (0 != session->mTempReportIntervel)
		{
			if (currentTimeMillis()/1000 - session->mTempReportStartTime > session->mTempReportValidTime)
			{
				session->mTempReportIntervel = 0;
			}
			else
			{
				reportTimeInterval = session->mTempReportIntervel;
			}
		}

		// 每隔1s获取报警/状态变化量队列，有数据则上报，无则根据上报策略计算上报
		if (getUptime() - lastReportTime2 >= 1)//(currentTimeMillis() - lastReportTime2 >= 1000)
		{
		...
		{
			if (getUptime() - lastReportTime >= reportTimeInterval)//(currentTimeMillis() - lastReportTime >= reportTimeInterval*1000)
			{
				lastReportTime = getUptime();//currentTimeMillis();
				report = 1;
			}
		}
		...
		if (report)
		{
			...
			lastReportTime2 = getUptime();//currentTimeMillis();

			controller->mCurrentTermState &= ~(1<<31); // 保留位作为adas/dms/bsd等报警标志
			...
			ext.mExtraInfoID = JT808_EXTRA_MSG_ID_WIRELESS_NETWORK_SIGNAL_STRENTH; // 4G信号强度
			ext.mExtraInfoLen = 1;
			ext.mExtraInfoData[0] = gNet4GRild.signalStrength;
			JT808Session_InsertLocationExtra(session, &ext);
			
			ext.mExtraInfoID = JT808_EXTRA_MSG_ID_GNSS_SATELLITE_NUM; // GPS卫星数
			ext.mExtraInfoLen = 1;
			ext.mExtraInfoData[0] = controller->mLocation.satellites;
			JT808Session_InsertLocationExtra(session, &ext);

			//ext.mExtraInfoID = JT808_EXTRA_MSG_ID_MILLAGE; // 里程数
			//ext.mExtraInfoLen = 4;
			//ext.mExtraInfoData[0] = (controller->mVehicalDriveDistance/100) >> 24;
			//ext.mExtraInfoData[1] = (controller->mVehicalDriveDistance/100) >> 16;
			//ext.mExtraInfoData[2] = (controller->mVehicalDriveDistance/100) >> 8;
			//ext.mExtraInfoData[3] = (controller->mVehicalDriveDistance/100);
			//JT808Session_InsertLocationExtra(session, &ext);
			

			if (checkTFCard(NULL))
			{
				ext.mExtraInfoID = JT808_VIDEO_RELATED_ALARM; // TF卡故障
				ext.mExtraInfoLen = 4;
				ext.mExtraInfoData[0] = 0x00;
				ext.mExtraInfoData[1] = 0x00;
				ext.mExtraInfoData[2] = 0x00;
				ext.mExtraInfoData[3] = 0x04;
				JT808Session_InsertLocationExtra(session, &ext);
				
				ext.mExtraInfoID = JT808_MEMORY_FAILURE_ALARM_STATUS; // TF卡故障
				ext.mExtraInfoLen = 2;
				ext.mExtraInfoData[0] = 0x00;
				ext.mExtraInfoData[1] = 0x01;
				JT808Session_InsertLocationExtra(session, &ext);
			}
...
void JT808Session_InsertLocationExtra(JT808Session_t* session, JT808MsgBody_0200Ex_t* ext)
{
	...
	else
	{
		...
		if (queue_list_push(&session->mLocationExtraInfoList, tmp))
		{
			free(tmp);
		}
		else
		{
			if (0x64 == ext->mExtraInfoID || 0x65 == ext->mExtraInfoID || 0x66 == ext->mExtraInfoID || 0x67 == ext->mExtraInfoID)
			{
				JT808Controller_t* controller = (JT808Controller_t*)session->mOnwer;
				controller->mCurrentTermState |= (1<<31); // 保留位作为adas/dms/bsd等报警标志
			}
...
{
	JT808Session_SendMsg(session, 0x1005, data, sizeof(JT808MsgBody_1005_t), 0);
}
...
void* JT808Session_UploadBlindLocationData(void* arg)
{
	...
	int timestop = JT808BlindUpload_GetCacheNewestGpsTime(&session->mBlineGpsData);
	int timestart = 0;
	...
	JT808SESSION_PRINT(session, "blind data timestop %d.", timestop);
...
void* JT808Session_MainProcess(void* arg)
{
	...
	JT1078MediaFiles_PlaybackInit(session);

	JT808ActiveSafety_UploadInit(&session->mAlarmUpload, session);
    
	JT808MediaEvent_Init(&session->mMediaEvent, session);
